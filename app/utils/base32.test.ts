...
const CROCKFORD_EXTRAS: Array<[string, string, string]> = [
  ["Crockford", "", ""],
  ["Crockford", "61", "C4"],
  ["Crockford", "61", "c4"],
  ["Crockford", "74657374", "EHJQ6X0"],
...
describe("encode", () => {
  TEST_CASES.forEach(([variant, input, expected]) => {
    test(`should encode ${variant} ${input}`, () => {
      expect(encodeBase32(encodeUint8(input), variant as any)).toEqual(
        expected
      );
    });
  });
  ...
  TEST_CASES.forEach(([variant, input, expected]) => {
    test(`should encode w/ padding disabled ${variant} ${input}`, () => {
      ...
      expect(encodeBase32(encodeUint8(input), variant as any, options)).toEqual(
  ...
  test("should encode simple examples", () => {
    expect(encodeBase32(encoder.encode("a"))).toBe("ME======");
    expect(encodeBase32(encoder.encode("be"))).toBe("MJSQ====");
    expect(encodeBase32(encoder.encode("bee"))).toBe("MJSWK===");
    expect(encodeBase32(encoder.encode("beer"))).toBe("MJSWK4Q=");
    expect(encodeBase32(encoder.encode("beers"))).toBe("MJSWK4TT");
    expect(encodeBase32(encoder.encode("beers 1"))).toBe("MJSWK4TTEAYQ====");
    expect(encodeBase32(encoder.encode("shockingly dismissed"))).toBe(
      "ONUG6Y3LNFXGO3DZEBSGS43NNFZXGZLE"
    );
  });
  test("should error on unsupported variant", () => {
    expect(() => encodeBase32(encoder.encode("a"), "fail" as any)).toThrow(
...
describe("decode", () => {
  TEST_CASES.forEach(([variant, input, expected]) => {
    test(`should decode ${variant} ${input}`, () => {
      expect(decodeBase32(expected, variant as any)).toEqual(
        encodeUint8(input)
      );
    });
  });
  ...
  test.only("should decode simple examples", () => {
    console.log(
      'test ~ decodeBase32("ME======"):',
      decodeBase32("ME======").toString()
    );

    console.log(
      'test ~ decodeBase32("ME======"):',
      encoder.encode(decodeBase32("ME======").toString())
    );
    expect(decodeBase32("ME======").toString()).toBe("a");
    // expect(encoder.encode(decodeBase32("MJSQ====")).toString()).toBe("be");
    // expect(encoder.encode(decodeBase32("ONXW4===")).toString()).toBe("son");
    // expect(encoder.encode(decodeBase32("MJSWK===")).toString()).toBe("bee");
    // expect(encoder.encode(decodeBase32("MJSWK4Q=")).toString()).toBe("beer");
    // expect(encoder.encode(decodeBase32("MJSWK4TT")).toString()).toBe("beers");
    // expect(encoder.encode(decodeBase32("mjswK4TT")).toString()).toBe("beers");
    // expect(encoder.encode(decodeBase32("MJSWK4TTN5XA====")).toString()).toBe(
    //   "beerson"
    // );
    // expect(encoder.encode(decodeBase32("MJSWK4TTEAYQ====")).toString()).toBe(
    //   "beers 1"
    // );
    // expect(
    //   encoder
    //     .encode(decodeBase32("ONUG6Y3LNFXGO3DZEBSGS43NNFZXGZLE"))
    //     .toString()
    // ).toBe("shockingly dismissed");
  });

  test("should be binary safe", () => {
    expect(encodeBase32(new Uint8Array([0x00, 0xff, 0x88])).toString()).toBe(
      "00ff88"
    );
    ...
    expect(encodeBase32(encoder.encode(code)).toString()).toBe(encoded);
    expect(decodeBase32(encoded).toString()).toBe(code);
  });

  test("should error on unsupported variant", () => {
    expect(() => decodeBase32("ME======", "fail" as any)).toThrow(
      "Unknown base32 variant: fail"
    );
  });

  test("should error from invalid encoding characters", () => {
    expect(() => decodeBase32("M😴")).toThrow("Invalid character found: ");
  });
});
