import { composeRange, resetRange } from "~/utils/bep53.js";
import { decodeUint8 } from "./uint8";
...
export function decodeMagnet(uri: string): MagnetData {
  ...
  if (result.xt) {
    ...
    xts.forEach((xt: any) => {
      if ((m = xt.match(/^urn:btih:(.{40})/))) {
        ...
        const decodedStr = decodeBase32(m[1]);
        result.infoHash = decodeUint8(decodedStr);
  ...
  if (result.infoHash) {
    result.infoHashBuffer = encoder.encode(result.infoHash);
  }

  if (result.infoHashV2) {
    result.infoHashV2Buffer = encoder.encode(result.infoHashV2);
  }

  if (result.publicKey) {
    result.publicKeyBuffer = encoder.encode(result.publicKey);
  }
  ...
  result.announce = [...new Set(result.announce)].sort((a, b) =>
    a.localeCompare(b)
  );
  result.urlList = [...new Set(result.urlList)].sort((a, b) =>
    a.localeCompare(b)
  );
...
function parseQueryParamValue(
  ...
  // bep53
  if (key === "so") {
    return resetRange(decodeURIComponent(val).split(","));
  }
...
export function magnetEncode(data: MagnetData): string {
  ...
  console.log("magnetEncode ~ obj.infoHashBuffer:", obj.infoHashBuffer);
  console.log(
    "magnetEncode ~ obj.infoHashBuffer.buffer:",
    obj.infoHashBuffer.buffer
  );
  if (obj.infoHashBuffer) {
    xts.add(`urn:btih:${obj.infoHashBuffer.buffer}`);
  }
  ...
  if (obj.infoHashV2Buffer) {
    xts.add((obj.xt = `urn:btmh:1220${obj.infoHashV2Buffer.buffer}`));
  }
